...
#endif
{
	...
	std::string loader = ini.GetString( "config", "loader", loader );

	ldr = fopen(loader.c_str(),"rb");

/*
#if defined(_STORAGE_rpg)
	ldr = fopen("fat0:/__rpg/rpgloader.nds", "rb");
#elif defined(_STORAGE_r4)
	ldr = fopen("fat0:/patch/loader.bin", "rb");
#elif defined(_STORAGE_ak2i)
	ldr = fopen("fat0:/__rpg/ak2iloader.nds","rb");
#elif defined(_STORAGE_r4idsn)
	ldr = fopen("fat0:/__rpg/r4idsnloader.nds","rb");
#endif
*/
